...
#define RTC_DEVICE "/dev/rtc16"
#define ALARM_ID 0
#define ALARM_TIME 180  // Alarm time in seconds (3 minutes)
...
int main(int argc, char *argv[])
{
  ...
  // Setup signal handler for the alarm
  act.sa_sigaction = alarm_handler;
  act.sa_flags = SA_SIGINFO;
  ...
  setrel.id = ALARM_ID;
  setrel.pid = getpid();
  setrel.reltime = ALARM_TIME;
  setrel.event.sigev_notify = SIGEV_SIGNAL;
  setrel.event.sigev_signo = SIGALRM;
  setrel.event.sigev_value.sival_int = ALARM_ID;

  if (ioctl(fd, RTC_SET_RELATIVE, (unsigned long)&setrel) < 0)
  {
